...
public class StreamingClient extends GenericProtocolExtension {
    ...
    public StreamingClient(String protoName, Properties properties) throws Exception{
        ...
        **/
        System.out.println("CLIENT STARTED "+connectionProtoHost);
    ...
    private void executeAux(int readAbleBytes, byte [] data, Host from, BabelStreamDeliveryEvent event){
        if(timeStart==0){
        ...
        try {
            if(event==null){
                ...
                event.babelOutputStream.readBytes(fileOutputStream);
            }
            if(iHaveFile.fileLength==totalReceived){
    ...
    void execute(BabelStreamDeliveryEvent event){
        executeAux(event.babelOutputStream.readableBytes(),null,event.getFrom(),event);
    }
    ...
    private void uponFileByets(FileBytesMessage msg, Host from, short sourceProto, int channelId, String streamId) {
        executeAux(msg.dataLength,msg.data,from,null);
    }
    ...
    public void init(Properties properties) throws HandlerRegistrationException, IOException {
        ...
        setupTimer(new BroadcastTimer(),30000);
    }
    ...
    public int createConnectionChannel(String proto, String address, String port,Properties properties) throws IOException {
        ...
        p.getRight().setProperty(NewChannelsFactoryUtils.SERVER_THREADS,properties.getProperty("SERVER_THREADS"));
